...
**TorchMix** provides the `Component` class, which a drop-in replacement for
...
These dataclasses as configurations is so-called **Structured Configs**.
...
You can then directly register this structured configs into `hydra`'s `ConfigStore`
...
In deep learning, models are always composed of multiple nested sub-models.
However, writing configurations for these nested modules can be
even more confusing and time-consuming. Let's say we have these models
...
Let me write the configurations for this setting:
...
So, `Component` allows you to confidently write arbitrarily nested modules
